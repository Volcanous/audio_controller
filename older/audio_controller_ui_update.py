# -------------------------------------------------
#  audio_controller.py  (UI update: tick labels beside sliders & 1% smooth movement)
# -------------------------------------------------
import os
import json
import time
...
class AudioControllerApp(ctk.CTk):
    ...
    # -------------------------------------------------
    # Center section (sliders with ticks on the right)
    # -------------------------------------------------
    def _create_center_sliders(self):
        ...
        for i, hz in enumerate(hz_labels):
            ...
            s = ctk.CTkSlider(
                ...
                from_=0,
                to=100,
                number_of_steps=100,  # smoother movement (1% increments)
                width=20,
                height=250,
            ...
            # Tick labels beside slider (right side)
            tick_frame = ctk.CTkFrame(inner_frame, fg_color="#2b2b2b")
            tick_frame.pack(side="left", padx=5)
            for v in tick_values:
                ctk.CTkLabel(tick_frame, text=str(v), font=("Helvetica", 12)).pack()
    ...
    def _populate_devices(self):
        try:
            ...
            names = list(dict.fromkeys([d["name"] for d in devices if d["max_output_channels"] > 0]))
