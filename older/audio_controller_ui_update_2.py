# -------------------------------------------------
#  audio_controller.py  (UI update: spaced tick labels, enabled devices only, +bass/treble/gain sliders)
# -------------------------------------------------
import os
import json
import threading
...
def apply_slider_filters(segment: AudioSegment, sliders):
    ...
    filtered = segment
    for val, fc in zip(sliders[:10], centres):
        bw = fc * (0.10 + val / 1000)
        low = max(fc - bw, 20)
        high = fc + bw
        filtered = high_pass_filter(filtered, low)
        filtered = low_pass_filter(filtered, high)
    # Bass, Treble, Gain adjustments
    bass, treble, gain = sliders[10:]
    filtered += int(gain - 50)  # simple gain adjustment
    return filtered
...
class AudioControllerApp(ctk.CTk):
    ...
    # -------------------------------------------------
    # Center section (main sliders + Bass/Treble/Gain)
    # -------------------------------------------------
    def _create_center_sliders(self):
        ...
        for i, hz in enumerate(hz_labels):
            ...
            inner_frame = ctk.CTkFrame(frame, fg_color="#2b2b2b")
            inner_frame.pack()

            s = ctk.CTkSlider(inner_frame, from_=0, to=100, number_of_steps=100, width=20, height=250, orientation="vertical", fg_color="#ffffff", button_color="#088888", button_hover_color="#0aa3a3")
            s.set(50)
            s.pack(side="left")

            tick_frame = ctk.CTkFrame(inner_frame, fg_color="#2b2b2b")
            tick_frame.pack(side="left", padx=10, fill="y", expand=True)
            for v in tick_values:
                lbl = ctk.CTkLabel(tick_frame, text=str(v), font=("Helvetica", 12))
        ...
        # Add Bass / Treble / Gain sliders
        extra_labels = ["Bass", "Treble", "Gain"]
        for j, name in enumerate(extra_labels):
            frame = ctk.CTkFrame(center, fg_color="#2b2b2b")
            frame.grid(row=1, column=j + 3, padx=30, pady=(20, 0))

            val_label = ctk.CTkLabel(frame, text="50%", font=("Helvetica", 12, "bold"))
            val_label.pack(pady=(0, 3))

            s = ctk.CTkSlider(frame, from_=0, to=100, number_of_steps=100, width=20, height=200, orientation="vertical", fg_color="#ffffff", button_color="#088888", button_hover_color="#0aa3a3")
            s.set(50)
            s.pack(pady=(5, 5))

            ctk.CTkLabel(frame, text=name).pack(pady=(5, 0))

            s.bind("<ButtonRelease-1>", lambda e, sl=s, lb=val_label: lb.configure(text=f"{int(sl.get())}%"))
            s.bind("<B1-Motion>", lambda e, sl=s, lb=val_label: lb.configure(text=f"{int(sl.get())}%"))
    ...
    # -------------------------------------------------
    # Device filtering (enabled only)
    # -------------------------------------------------
    def _populate_devices(self):
        try:
            devices = sd.query_devices()
            names = [d["name"] for d in devices if d["max_output_channels"] > 0 and not d.get("hostapi") is None]
